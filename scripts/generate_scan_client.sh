...
CLIENT_DIR="${ROOT_DIR}/gooddata-scan-client"
...
docker run --rm \
    ...
    -g python \
    --package-name gooddata_scan_client \
    --additional-properties packageVersion=0.3.3,packageName=gooddata_scan_client \
    -i https://staging.anywhere.gooddata.com/api/schemas/scan \
    -o /local/gooddata-scan-client
...
#
sed -i -e 's/_self/_self_sanitized/g' "${CLIENT_DIR}/gooddata_scan_client/model_utils.py"
